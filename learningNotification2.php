...
if (!($stmt = $conn->prepare("SELECT NotificationsState FROM User WHERE DeviceToken = ? LIMIT 0,1"))) {
...
if ($result > 0) {
    ...
    _dvptLog('$row["NotificationsState"]='.$user_notification_state);
} else {
    _dvptLog("0 results");
}
